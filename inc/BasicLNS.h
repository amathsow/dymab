...
struct BanditStats {
    ...
    vector<std::deque<double>> recent_rewards;  
    ...
    // Add constructor
    BanditStats(int numberOfDestroyHeuristics = 0) : banditIndex(0) {
        if (numberOfDestroyHeuristics > 0) {
            recent_rewards.resize(numberOfDestroyHeuristics);
            destroy_weights.resize(numberOfDestroyHeuristics, 0.0);
            destroy_weights_squared.resize(numberOfDestroyHeuristics, 0.0);
            destroy_counts.resize(numberOfDestroyHeuristics, 0);
        }
    }
};
...
protected:
    ...
    int numberOfDestroyHeuristics=8;
