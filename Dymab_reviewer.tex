...
\]
Because $\mathcal{N}$ is a set, duplicate entries are automatically handled, and there is no need to choose \emph{among} conflicting agents: all identified conflicts are included until $|\mathcal{N}| \geq M$, at which point the walk terminates. This is already captured in Algorithm~1.
\end{authorbox}

\begin{changebox}
\textbf{Changes in paper:} Clarification paragraph added in Section~4.1.2 explaining the set-union mechanism.
\end{changebox}
...
\begin{authorbox}
\textbf{Authors' response:} Both observations are valid, and we address them as follows.

\paragraph{Naming.} The inner walk (Algorithm~3) selects the next location greedily based on entropy score rather than uniformly at random. However, randomness is still present via the Boltzmann-based seed agent selection in the outer algorithm (Algorithm~2), which samples the seed agent proportionally to its entropy score. We have renamed Algorithm~3 from ``Location Entropy Random Walk'' to \textbf{``Entropy-Guided Walk''} to more accurately reflect that the movement is entropy-guided rather than random.

\paragraph{Overflow beyond $M$.} The reviewer is correct that the original algorithm description did not explicitly guard against $|\mathcal{N}|$ exceeding $M$. The safeguard was present in the implementation but not reflected in the pseudocode. We have now added an explicit check after the \textsc{ConflictingAgents} call: if $|\mathcal{N}| \geq M$, the loop breaks immediately. This ensures the returned neighborhood is always of size at most $M$.
\end{authorbox}

\begin{changebox}
\textbf{Changes in paper:} Algorithm~3 renamed to ``Entropy-Guided Walk''. Explicit overflow guard ($|\mathcal{N}| \geq M \Rightarrow$ break) added to the algorithm.
\end{changebox}
...
\begin{authorbox}
\textbf{Authors' response:} We thank the reviewer for these specific and constructive suggestions. We address each point in turn.

\paragraph{LaCAM specification.}
We clarify in Section~6 that the LaCAM baseline uses the \textbf{original LaCAM} implementation~\citep{LaCam} (AAAI-23, Okumura), which is a complete, search-based anytime algorithm known for strong scalability.

\paragraph{LaCAM*.}
LaCAM*~\citep{LaCamStar} (IJCAI-23, Okumura) is an extension of LaCAM that provides eventual optimality guarantees while retaining the anytime property. We have added \textbf{LaCAM*} as an additional baseline in the large-scale experiment (Table~4), using the official open-source implementation. This allows a direct comparison between DyMAB and both LaCAM variants across increasing numbers of agents.

\paragraph{CBSH2-RTC and ECBS.}
CBSH2-RTC~\citep{CBSH2} and ECBS~\citep{ECBS} are bounded-suboptimal \emph{complete} solvers: they find a solution within a guaranteed suboptimality factor and then terminate. They do not continuously improve a solution within a time budget. Their scalability is also fundamentally limited to small-to-medium instances (typically below 300--500 agents), making them unsuitable as baselines in a large-scale anytime evaluation. We have added a clarifying sentence in Section~2 (Related Work) to explain this distinction.

\paragraph{Enhanced Table~4: increasing agent counts and runtime.}
Following the evaluation format in BALANCE~\citep{Balance}, we have restructured Table~4 to include results across \textbf{increasing numbers of agents} per map, providing a clearer view of how each algorithm scales. Both LaCAM and LaCAM* are included in this extended table, allowing the reviewer to assess the relative performance of anytime LNS-based methods (DyMAB, BALANCE) against search-based methods (LaCAM, LaCAM*) as problem size grows.
\end{authorbox}

\begin{changebox}
\textbf{Changes in paper:}
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Section~6:} LaCAM baseline specified as the original LaCAM~\citep{LaCam} (AAAI-23).
  \item \textbf{Table~4 extended:} LaCAM*~\citep{LaCamStar} (IJCAI-23) added as a new baseline; results reported across increasing agent counts per map.
  \item \textbf{Section~2 (Related Work):} Sentence added explaining that CBSH2-RTC and ECBS are bounded-suboptimal complete solvers, not anytime algorithms, and therefore not direct baselines in this evaluation.
\end{itemize}
\end{changebox}
...
\begin{enumerate}[leftmargin=2em, label=\textbf{C\arabic*}]
  ...
  \item \textbf{SOTA comparison extended:} LaCAM baseline specified as original LaCAM~\citep{LaCam} (AAAI-23); LaCAM*~\citep{LaCamStar} (IJCAI-23) added as a new baseline; Table~4 extended with increasing agent counts per map. Section~2 notes that CBSH2-RTC/ECBS are complete solvers, not anytime, and are therefore excluded. \emph{(Reviewer H, W5)}

  \item \textbf{Confidence intervals added} to all experimental tables. \emph{(Reviewer H, Minor 1)}

  \item \textbf{Section~2 (Related Work) extended:} contextual bandits discussed (W2); CBSH2-RTC and ECBS identified as bounded-suboptimal complete solvers (not anytime), explaining their exclusion from the comparison. \emph{(Reviewer H, W2, W5)}
