...
int BasicLNS::sampleDestroyHeuristic(BanditStats* stats)
{
    ...
    if (bandit_algorithm == RANDOM_BANDIT) {
        ...
        for (int index = 0; index < stats->destroy_weights.size(); ++index) {
            ...
            double explorationTerm = sqrt((2 * dynamicAlpha * log(iterationCount + 1)) / (count > 0 ? count : 1));
        ...
        iterationCount++;
        // Use polynomial decay for epsilonT
    ...
    //iterationCount++; // Increment the iteration count after selection
